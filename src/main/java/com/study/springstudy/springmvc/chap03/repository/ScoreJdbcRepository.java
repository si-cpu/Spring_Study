...
// ResultSet: sql실행 결과(조회)의 내용을 갖고 있는 객체,
// 타겟을 한 행씩 지목하면서 컬럼값을 가져올 수 있다.
// mapRow 메서드를 통해 jdbcTemplate한테 알려주자 --> 한행의 칼럼값을
// Score 객체로 포장하는 방법을
        @Override
    ...
    //데이터베이스 접속 객체(Connection)을 바로 활용하는 것이 가능 -> 미리 세팅을 다해놓음.
    //lombok의 Requi@RequiredArgsConstructor 아노테이션을 이용하여 무조건 초기화 되도록 유도
    ...
    public List<Score> findAll() {
        ...
        // 여러행이 조회 될때는 query()를 호출.
        // sql, RowMapper 인터페이스를 구현한 객체를 전달.
        // 조회된 내용을 어떤 방식으로 포장할 지를 알려줘야 한다. (테이블은 컬럼, 개수 등이 모두 다르기 떄문)
